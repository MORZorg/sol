...
int yygen( FILE* input, FILE* output )
{
	...
	{
		current_call = call_list->function;
		...
		current_call->push_pop.head->args[ 0 ].i_val = func_desc->function->formals_size;
		current_call->push_pop.head->args[ 1 ].i_val = func_desc->function->last_oid - func_desc->function->formals_size - 1;
		current_call->push_pop.head->next->args[ 0 ].i_val = func_desc->entry->address;
...
Code generate_intro_code( Symbol* base_function )
{
	...
	// NOTE Assumes the size of the code generated by make_push_pop is 3
	return concatenate_code( 3,
							 result,
							 make_push_pop( base_function->formals_size,
											base_function->last_oid - base_function->formals_size - 1,
											-1,
											result.size + 4 ),
		...
		case T_UNQUALIFIED_NONTERMINAL:
			switch( node->value.n_val )
			{
				...
				case N_FUNC_DECL:
				{
					...
					description->function = func_scope;
				...
				case N_FUNC_CALL:
				{
					...
					int chain = ( (Symbol*) scope->function )->nesting - description->function->nesting + 1;

					// Other parameters adjusted in post_processing
					Code actual_call = make_push_pop( 0, 0, chain, 0 );
				...
				case N_FOREACH_STAT:
				{
					...
					sprintf( loop_var->name, "%d", ( (Symbol*) scope->function )->last_oid );
					loop_var->oid = ( (Symbol*) scope->function )->last_oid++;
					...
					Node* loop_node = new_nonterminal_node( N_ASSIGN_STAT );
					loop_node->child = new_terminal_node( T_ID, node->child->value );
					loop_node->child->brother = new_nonterminal_node( N_INDEXING );
 ...
 * @return 
 */
Code make_push_pop( int formals_size, int locals_size, int chain, int entry )
{
	Code result = concatenate_code( 3,
									make_code_two_param( SOL_PUSH, formals_size, locals_size ),
									make_code_one_param( SOL_GOTO, entry ),
									make_code_no_param( SOL_POP ) );
	result.head->args[ 2 ].i_val = chain;
	return result;
}
...
void output_code( FILE* output, Code code )
{
	...
	for( current_stat = code.head; current_stat != NULL; current_stat = current_stat->next )
		switch( current_stat->op )
		{
			...
			// Two parameters and string operators
			case SOL_FREAD:
			case SOL_READ:
			...
			// Two parameters and int operators
			case SOL_PUSH:
